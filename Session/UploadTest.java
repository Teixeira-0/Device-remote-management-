...
public class UploadTest {
    ...
    void setup() throws NoSuchAlgorithmException, IOException, KeyManagementException, CertificateException, UnrecoverableKeyException, KeyStoreException, InterruptedException {
        ...
        ReadapMessage response = new ReadapMessage(ReadapCodes.VERSION, ReadapCodes.UPLOAD,new byte[]{0,0,0,0,0,0,0,10});
        out.write(response.toByteArrayRemainder());

        response = new ReadapMessage(ReadapCodes.VERSION, ReadapCodes.UPLOAD, new byte[]{0,0,0,0,0,0,0,10});
        out.write(response.toByteArrayRemainder());
    ...
    void integrationTest() throws IOException, InterruptedException {
        ...
        session.uploadData(in,out,"TLSProvider.java");

        Thread.sleep(1000);
        Assertions.assertTrue(new String(sent.getChunk(), 0, sent.getChunkLength(), StandardCharsets.UTF_8).isBlank());
